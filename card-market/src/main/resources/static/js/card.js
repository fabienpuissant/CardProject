...
/*****************************************************************************************************************************************/
	...
	function displayCards(cards){
		...
		for(i = 0; i < cards.length; i++){
			  ...
			  $.ajax({
				  url:"http://localhost:8082/CardService/" + cards[i],
				  type:"GET",
	...
	function addFocusListener(type){
		...
		$("#imgrow").on("click", ".cardchild" ,function(event){
		    ...
		    if($(event.target).closest("td").attr('class') == "cardbutton"){
		    	...
		    	$.ajax({
					  url:"http://localhost:8083/MarketService/" + type + "/" + json.name + "/" + id,
					  type:"GET",
					  success: function( data ){
						  if(data == true){
	...
	function updateMoney(){
		$.ajax({
			  url:"http://localhost:8081/UserService/money/" + json.name,
			  type:"GET",
			  success: function( data ){
					  $("#money").text(data);
			  }
	...
	function displaySellMarket(){
		...
		$.ajax({
			  url:"http://localhost:8082/CardService/cards/" + json.id,
			  type:"GET",
			  success: function( data ){
	...
	function displayBuyMarket(){
		...
		$.ajax({
			  url:"http://localhost:8082/CardService/user/BuyCards",
			  type:"GET",
			  success: function( data ){
				  cards = data.split('/');
				  displayCards(cards);
